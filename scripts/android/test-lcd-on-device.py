...
p=argparse.ArgumentParser();p.add_argument('--serial',required=True);p.add_argument('--live-upload',action='store_true',help='Explicitly send one synthetic parity report to the project support endpoint');p.add_argument('--suite',choices=['lcd','keypad','startup','security','dtc','firmware','firmware-download','support','vehicle','connection','updates','parity'],default='lcd');p.add_argument('--classes',type=pathlib.Path,help='Compiled application classes (defaults to the custom build)');p.add_argument('--package', choices=['com.opensaab.tech2','com.opensaab.tech2.headunit32'],default='com.opensaab.tech2');a=p.parse_args()
test_class={'startup':'StartupFlowInstrumentedTest','lcd':'NativeLcdPumpInstrumentedTest','keypad':'Tech2ControlsInstrumentedTest','security':'SecurityAccessInstrumentedTest','dtc':'DtcReportInstrumentedTest','firmware':'FirmwareInstrumentedTest','firmware-download':'FirmwareDownloadInstrumentedTest','support':'SupportReportInstrumentedTest','vehicle':'VehicleSessionInstrumentedTest','connection':'ConnectionReportInstrumentedTest','updates':'AppUpdatesInstrumentedTest','parity':'ReleaseParityInstrumentedTest'}[a.suite]
if a.live_upload and a.suite!='parity':raise SystemExit('--live-upload is only supported for parity')
if a.suite in ('startup','firmware-download','support','connection','parity') and not a.serial.startswith('emulator-'):raise SystemExit('This suite requires an Android emulator')
