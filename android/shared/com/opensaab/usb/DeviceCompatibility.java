...
public final class DeviceCompatibility {
    public static String report(Context context) {
        ...
        boolean arm64 = CompatibilityCheck.supportsArm64(Build.SUPPORTED_ABIS);
        ...
        text.append(CompatibilityCheck.verdict(Build.VERSION.SDK_INT, Build.SUPPORTED_ABIS, free));
        ...
        text.append("\n").append(arm64 ? "PASS" : "FAIL").append(" — Android supports arm64-v8a applications");
        if (!arm64) text.append("\nThe current emulator APK is ARM64-only. A 64-bit CPU running 32-bit Android is not sufficient. Retrying this APK will not fix the mismatch. A compatible build or device is needed.");
        ...
        text.append("\n\nThis checks requirements for the current ARM64 release, not complete vehicle compatibility. It does not test the USB adapter, firmware, server connection or available memory under load.");
