...
/** Grants the chosen share recipient read access to one frozen support bundle, never raw session logs. */
public final class SupportReportProvider extends ContentProvider {
    public boolean onCreate(){return true;}
    private File file(Uri uri) throws FileNotFoundException {
        if(!(getContext().getPackageName()+".support-reports").equals(uri.getAuthority()) || uri.getPathSegments().size()!=1)
            throw new FileNotFoundException("Not a support report");
        String name=uri.getLastPathSegment();
        if(!name.matches("android_support_[a-f0-9-]+\\.zip"))throw new FileNotFoundException("Not a support report");
        File parent=new File(getContext().getFilesDir(),"support-reports"), f=new File(parent,name);
