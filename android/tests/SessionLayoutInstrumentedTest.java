 ...
 public void onStart(){Bundle out=new Bundle();Activity current=null;try{
  for(String name:new String[]{"com.opensaab.tech2.MainActivity","com.opensaab.usb.ChipsoftUsbActivity"}){
   ...
   ui(()->{
    ...
    layouts(a);View root=a.getWindow().getDecorView();
    if(name.endsWith("MainActivity")){visible(root,"start-offline");check(text(root,"Run without an adapter")!=null,"Offline launch hidden");}visible(root,"tech2-navigation-hint");visible(root,"tech2-key-1");visible(root,"tech2-actions");visible(root,"session-menu");visible(root,"session-summary");
