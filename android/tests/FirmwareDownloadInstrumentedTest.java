...
public final class FirmwareDownloadInstrumentedTest extends Instrumentation {
    ...
    public void onStart(){Bundle result=new Bundle();int code=-1;Activity activity=null;boolean networkOff=false;
        try{
            ...
            visible(a,"Your Saab software");visible(a,"Internet is needed");
            ...
            else {check(store.missingSupport().equals("eprom.bin, opsys.dwn, candi.bin"),"Absent support not recorded");visible(a,"import eprom.bin");}
            shot("01-welcome");
            visible(a,"Download once.");
            ...
            click(a,"Download and continue");waitReady(a);visible(a,"Check your internet connection");check(!new File(store.firmware,"card.bin").exists(),"Offline failure created active card");shot("03-offline-retry");
            ...
            check(store.missing().isEmpty(),"Download did not finish setup automatically");
            waitForIdleSync();check(a.isFinishing()||a.isDestroyed(),"Ready first run must return automatically");shot("04-ready");
            ...
            main(()->spinner(resumed.getWindow().getDecorView()).setSelection(1));click(a,"Download and continue");waitReady(a);
