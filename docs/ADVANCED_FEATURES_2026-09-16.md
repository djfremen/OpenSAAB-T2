...
## Logo-screen shortcut follow-up

Review builds preview.18 / 100018 and headunit.12 / 100012 (source 5831108) recognize the explicit software-version / Press ENTER splash screen. All four shortcuts send one ENTER, wait for the main menu and continue normal navigation. Unchanged frames do not repeat the key; blank restart frames wait; manual takeover and unknown-menu handling remain unchanged.

Pure Java request/security/navigation fixtures pass, including all four targets from the splash, blank transition frames, duplicate-frame suppression and cancellation. Android security instrumentation passes against preview.18. ARM64 installed in place on Pixel 7 with native session already stopped; installed version and launcher controls checked. ARM32 built; no website publication. Connected-vehicle testing of the splash shortcut fix remains pending.

The owner reports successful manual security collection on preview.17. The Pixel receipt confirms pre-auth collected at 22:22:48 PDT, OpenSAAB post-auth received at 22:22:51 and written at 22:22:53 on September 16. Receipt, screenshots, package metadata and available tagged logcat were archived privately. Older success log events had rolled out; app-private native session files were not exported. A later USB cleanup warning was retained separately and is not evidence that the recorded post-auth import failed.
