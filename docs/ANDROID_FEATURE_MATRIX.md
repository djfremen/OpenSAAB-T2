...
| Feature | ARM64 preview.2 | ARM64 preview.3 | ARM32 headunit.3 | Setup 0.3.2 |
| --- | --- | --- | --- | --- |
...
Validation for preview.3: 210 Rust tests passed (9 ignored), Java host checks and architecture-isolation tests passed, 8 report-service tests passed. Android release instrumentation passed shared UI checks, report redaction/export and error contracts, offline native startup/stop, and real resource sampling. Live Android upload returned a private receipt; authenticated retrieval matched the submitted report exactly and anonymous retrieval was denied. No vehicle was connected for these tests. Physical-device installation and final published hashes are recorded in the release build receipt.

ARM64 preview.4 follows preview.3 with a report-review layout correction: a bounded scrolling preview reserves space for all three actions. Physical Pixel inspection exposed clipping in the platform dialog after preview.3 publication. A regression check now requires all three actions to be fully visible together. ARM32 remains headunit.3; this shared-source fix is available for its next independent release.
