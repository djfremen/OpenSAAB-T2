...
use crate::{
    can_adapter::{nano_route, CompletionSource, Event},
...
pub fn allowed_for_profile(controller: usize, tx: &CanTransmission, profile: Profile) -> bool {
    ...
    let Ok(route) = nano_route(controller, tx) else {
        return false;
    };
    let id = tx.frame.id;
...
impl CommandGate {
    pub fn allowed(
        ...
        if nano_route(controller, tx).is_err() {
            return false;
        }
        let d = &tx.frame.data;
...
fn encode_authorized(
    ...
    let route = nano_route(controller, tx)?;
